...
fn test_stating_something() {
    assert_eq!(
        "Whatever.",
        bob::reply("Tom-ay-to, tom-aaaah-to.")
    );
}
...
fn test_ask_numeric_question() {
    assert_eq!(
        "Sure.",
        bob::reply("You are, what, like 15?")
    );
}
...
fn test_exclaiming() {
    assert_eq!(
        "Whatever.",
        bob::reply("Let's go make out behind the gym!")
    );
}
...
fn test_statement_containing_question_mark() {
    assert_eq!(
        "Whatever.",
        bob::reply("Ending with ? means a question.")
    );
}
