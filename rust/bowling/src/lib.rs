...
trait Frame: Deref {
    // fn iter() -> Box<Iterator<Item=u32>>;
    fn push(&mut self, roll: u32) -> Result<(), ()>;
}

// impl Deref for Frame<Target = [u32]> {
//     fn deref<'a>(&'a self) -> &'a [T] { self.rolls.as_slice() }
// }
...
impl Deref for NormalFrame {
    type Target = [u32];
    fn deref<'a>(&'a self) -> &'a [u32] { self.rolls.as_slice() }
}
...
impl Deref for LastFrame {
    type Target = [u32];
    fn deref<'a>(&'a self) -> &'a [u32] { self.rolls.as_slice() }
}

pub struct BowlingGame {
    frames: Vec<Box<Frame<Target = [u32]>>>,
    active_frame: Box<Frame<Target = [u32]>>
}
...
impl BowlingGame {
    ...
    pub fn roll(&mut self, _pins: u32) -> Result<(), ()> {
        unimplemented!()
    }
