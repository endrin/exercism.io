...
fn to_double_byte() {
    assert_eq!(
        &[0x81, 0x00],
        vlq::to_bytes(&[0x80]).as_slice()
    );
    assert_eq!(
        &[0xc0, 0x00],
        vlq::to_bytes(&[0x2000]).as_slice()
    );
    assert_eq!(
        &[0xff, 0x7f],
        vlq::to_bytes(&[0x3fff]).as_slice()
    );
}
...
fn to_triple_byte() {
    assert_eq!(
        &[0x81, 0x80, 0x00],
        vlq::to_bytes(&[0x4000]).as_slice()
    );
    assert_eq!(
        &[0xc0, 0x80, 0x00],
        vlq::to_bytes(&[0x10_0000]).as_slice()
    );
    assert_eq!(
        &[0xff, 0xff, 0x7f],
        vlq::to_bytes(&[0x1f_ffff]).as_slice()
    );
}
...
fn from_bytes() {
    assert_eq!(
        &[0x7f],
        vlq::from_bytes(&[0x7f]).unwrap().as_slice()
    );
    assert_eq!(
        &[0x2000],
...
fn to_bytes_multiple_values() {
    assert_eq!(
        &[0x40, 0x7f],
        vlq::to_bytes(&[0x40, 0x7f]).as_slice()
    );
    assert_eq!(
